...
//    public static final int MAX_NUMBER_OF_OPTIMIZATIONS = -1;  // Only used for debugging
    ...
    private boolean workGenerate(CodeBase code) {
        ...
        // Set potential jump targets:
        for(int i = 3; i <= spec.maxBytesAlloedToJumpAhead; i++) {
            // Notice that we map "i" -> "codeStartAddress + i" for relative jumps because
            // ops are preassembled to bytes, assuming that they start at address "0". So,
            // they should be considered as offsets
            if (i < 130) {
                spec.allowedJrTargets.add(Pair.of((Integer)i,
                                                  Expression.constantExpression(spec.codeStartAddress + i, config)));
            }
            spec.allowedJpTargets.add(Pair.of((Integer)spec.codeStartAddress + i,
                                              Expression.constantExpression(spec.codeStartAddress + i, config)));
        }

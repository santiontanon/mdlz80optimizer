 ...
 * @author santi
 */
public class Z80nBinaryGeneratorTest {
        
    private final MDLConfig config;
    private final CodeBase code;

    public Z80nBinaryGeneratorTest() {
        config = new MDLConfig();
        code = new CodeBase(config);
    }

    @Test public void test1() throws Exception { Assert.assertTrue(test("data/generationtests/z80n-ops.asm", null, 
                                                                        "data/generationtests/z80n-ops-expected.bin")); }


    private boolean test(String inputFile, String dialect, String expectedOutputFile) throws Exception
    {
        if (dialect == null) {
            Assert.assertTrue(config.parseArgs(inputFile,"-cpu","z80n"));
        } else {
            Assert.assertTrue(config.parseArgs(inputFile,"-cpu","z80n","-dialect",dialect));
        }
        Assert.assertTrue(
